...
const Payment = () => {
  ...
  const { paymentAmount } = useGlobals();
  ...
  useEffect(() => {
    fetch("/api/create-payment-intent", {
      ...
      body: JSON.stringify({ amount: convertToSubcurrency(paymentAmount) }),
    })
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await stripe.confirmPayment({
      ...
      clientSecret,
      confirmParams: {
        return_url: `${process.env.BASE_URL}/payment-success?amount=${paymentAmount}`,
      },
    });
